...
class Qubit:                                           #creates the qubit class
    ...
    def __setitem__(self: "Qubit", index: int, new_qubit: "Qubit") -> None:
        ...
        if new_qubit.dim == 2:
            if self.n == 2:
                if index == 0:
                    replaced_qubit, state_B = self.decompose_state(index)
                    new_qubit = new_qubit % state_B
                elif index == 1:
                    state_A, replaced_qubit = self.decompose_state(index)
                    new_qubit = state_A % new_qubit
            else:
                state_A, replaced_qubit, state_B = self.decompose_state(index)
                new_qubit = state_A % new_qubit % state_B 
            self.rho = new_qubit.rho
        else:
            raise QuantumStateError(f"The new rho needs to be the same dimension as the old rho, not {self.dim} and {new_qubit.dim}")
    ...
    def decompose_state(self, qubit_index: int) -> tuple["Qubit", "Qubit", "Qubit"]:
        ...
        if qubit_index is not None:
            ...
            if isinstance(qubit_index, int):
                ...
                if self.n == 2:
                    if qubit_index == 0:
                        state_B = self.partial_trace(1, 0)
                        return isolated_rho, state_B
                    elif qubit_index == 1:
                        state_A = self.partial_trace(0, 1)
                        return state_A, isolated_rho
                else:
                    state_A = self.partial_trace(0, self.n - qubit_index)
                    state_B = self.partial_trace(qubit_index + 1, 0)
                return state_A, isolated_rho, state_B
...
print(test_qub.partial_trace(1,1))
print(test_qub.partial_trace(0,2))
print(test_qub.partial_trace(2,0))